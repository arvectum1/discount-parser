...
def install_dynamic_offer_fields() -> None:
    ...
    it wraps the universal persistence gate. Dynamic merchant enrichment may run
    before persistence, but it must never bypass universal offer-quality checks.
    """
    ...
    if getattr(current, _QUALITY_MARKER, False) or getattr(current, "__name__", "") == "_persist_with_quality_gate":
